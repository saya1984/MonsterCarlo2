...
class Tree(object):
    ...
    def update(self, path, score, duration = 0, terminal = False, tree_blocks = ""):
        ...
        def run(self):
            ...
            while True:           
                ...
                while True:
                    ...
                    response = json.loads(result)
                    path, score, tree_blocks = response['path'], response['score'], response['tree_blocks']
                    
                    tree.update(path, score, duration, tree_blocks = tree_blocks)
                    if callback and print_delay >= 100:
                        print_delay = 0
                        callback(tree)
                    print_delay += 1
                ...
                with lock:
                    results.append({"path": tree.best_path, "random_seed": tree_seed,"score": tree.best_score,"blocks":tree.tree_blocks})
